    ...
    function updateFontTargets(targets, family, weight, style) {
      targets.forEach(el => { 
    ...
    function initTypingControls() {
      ...
      document.querySelectorAll('.type-tester').forEach(section => {
        ...
        const widthSlider = section.querySelector('.width');
        ...
        if (widthSlider) {
          if (!widthSlider.value) {
            widthSlider.value = 100;
          }
          textArea.style.fontVariationSettings = `"wdth" ${widthSlider.value}`;
...
// ====== TYPEWRITER EFFECT FOR KALDANIT PAGE ======
function initTypewriterEffect() {
  // Only apply to Kaldanit page
  if (!document.body.classList.contains('kaldanit-page')) return;
  
  // Wait for header to load, check multiple times
  let attempts = 0;
  const maxAttempts = 20; // Check for up to 2 seconds (20 * 100ms)
  
  function tryInitTypewriter() {
    const coverTitleDiv = document.querySelector('.cover-title div');
    
    if (coverTitleDiv && coverTitleDiv.textContent) {
      const fullText = coverTitleDiv.textContent.trim();
      if (fullText) {
        // Clear the text initially (CSS already hides it)
        coverTitleDiv.textContent = '';
        // Make it visible and add active class
        coverTitleDiv.classList.add('typewriter-active');
        
        // Typewriter effect
        let charIndex = 0;
        const typeSpeed = 100; // milliseconds per character
        const cursorChar = '|';
        const textWithoutCursor = fullText.replace(cursorChar, '');
        let cursorSpan = null;
        let blinkInterval = null;
        
        function startBlinking() {
          if (cursorSpan && !blinkInterval) {
            let isVisible = true; // Start visible
            // Blink function
            const blink = () => {
              cursorSpan.style.opacity = isVisible ? '1' : '0';
              isVisible = !isVisible;
            };
            // Start with cursor visible
            cursorSpan.style.opacity = '1'; // Start visible
            // First blink happens immediately, then continue every 500ms
            blink(); // First blink immediately
            blinkInterval = setInterval(blink, 500); // Then continue every 500ms
          }
        }
        
        function typeChar() {
          if (charIndex < textWithoutCursor.length) {
            // Type the text without cursor
            coverTitleDiv.textContent = textWithoutCursor.substring(0, charIndex + 1);
            charIndex++;
            setTimeout(typeChar, typeSpeed);
          } else {
            // Add the cursor and start blinking immediately (no delay)
            cursorSpan = document.createElement('span');
            cursorSpan.className = 'typewriter-cursor';
            cursorSpan.textContent = cursorChar;
            coverTitleDiv.appendChild(cursorSpan);
            
            // Start blinking animation immediately - starts hidden, then appears
            startBlinking();
          }
        }
    ...
    if (attempts < maxAttempts) {
      attempts++;
      setTimeout(tryInitTypewriter, 100);
    }
